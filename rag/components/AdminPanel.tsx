...
import { 
  Upload, Trash2, FileText, Plus, Database, Info, File, X, 
  Loader2, CheckCircle2, AlertCircle, Users, Shield, 
  MessageSquare, UserPlus, Ban, ShieldCheck, Search 
} from 'lucide-react';
          ...
          <div className="flex justify-between items-center bg-white dark:bg-[#12161c] p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-sm">
            ...
            <div className="flex items-center gap-6">
              <div className="text-right">
                <p className="text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Total Nodes</p>
                <p className="text-xl font-black text-[#4ade80]">{users.length}</p>
              </div>
            </div>
          </div>
              ...
              <div key={u.id} className="bg-white dark:bg-[#12161c] p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 hover:border-slate-300 dark:hover:border-white/10 transition-all flex items-center justify-between group shadow-sm">
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium">{u.email} • Joined {new Date(u.createdAt).toLocaleDateString()}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="p-3 bg-slate-50 dark:bg-white/5 hover:bg-amber-500/20 text-slate-400 hover:text-amber-500 rounded-xl transition-all"
                  ...
                  <button 
                    ...
                    className={`p-3 bg-slate-50 dark:bg-white/5 rounded-xl transition-all ${u.status === 'blocked' ? 'text-[#4ade80] hover:bg-[#4ade80]/20' : 'text-red-500 hover:bg-red-500/20'}`}
                    title={u.status === 'blocked' ? "Activate Node" : "Block Node"}
                  >
                    {u.status === 'blocked' ? <CheckCircle2 size={18} /> : <Ban size={18} />}
                  </button>
                  <button 
                    onClick={() => handleDeleteUser(u.id)}
                    className="p-3 bg-slate-50 dark:bg-white/5 hover:bg-red-500 text-slate-400 hover:text-white rounded-xl transition-all"
                    title="Purge Node"
                  >
                    <Trash2 size={18} />
                  </button>
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
           <div className="flex justify-between items-center mb-8">
             ...
             <button 
                ...
                className="bg-[#4ade80] text-black px-6 py-3 rounded-2xl font-black uppercase text-xs tracking-widest flex items-center gap-2 hover:scale-105 transition-all shadow-lg shadow-[#4ade80]/20"
              >
           ...
           {isAdding && (
             <div className="bg-white dark:bg-[#12161c] p-8 rounded-[2.5rem] border border-[#4ade80]/20 mb-8 animate-in slide-in-from-top-4 shadow-xl">
                <input 
                  ...
                  className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 rounded-2xl p-4 text-slate-900 dark:text-white mb-4 outline-none focus:border-[#4ade80]/40 transition-all"
                />
                <textarea 
                  rows={6}
                  placeholder="Knowledge parameters..."
                  value={newContent}
                  onChange={(e) => setNewContent(e.target.value)}
                  className="w-full bg-slate-50 dark:bg-black/40 border border-slate-200 dark:border-white/5 rounded-2xl p-4 text-slate-900 dark:text-white mb-6 outline-none focus:border-[#4ade80]/40 transition-all resize-none"
                />
                <div className="flex justify-end gap-3">
                  ...
                  <button 
                    ...
                    className="bg-[#4ade80] text-black px-8 py-3 rounded-2xl font-black uppercase text-xs tracking-widest flex items-center gap-2"
                  >
                    {isProcessing ? <Loader2 size={18} className="animate-spin" /> : "Commit Index"}
                  </button>
                </div>
             </div>
           )}
             ...
             {docs.map(doc => (
               <div key={doc.id} className="bg-white dark:bg-[#12161c] p-6 rounded-[2rem] border border-slate-200 dark:border-white/5 hover:border-[#4ade80]/20 transition-all shadow-sm">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <button onClick={() => {vectorStore.deleteDocument(doc.id); setDocs(vectorStore.getDocuments());}} className="text-slate-400 hover:text-red-500 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
               </div>
